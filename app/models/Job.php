...
class Job extends Model{
    ...
    public function __construct()
    {
        ...
        $this->completed_id = $this->getStatusId('completed');
        $this->getStatusses();
    }
    ...
    {
        $db = Database::openConnection();
        $statusses = $db->queryData("SELECT id, name FROM job_status ORDER BY name");
    ...
    {
        $db = Database::openConnection();
        return ($db->queryValue('job_status', array('name' => $status)));
    }

    public function getStatusName($status_id)
    {
        $db = Database::openConnection();
        return ($db->queryValue('job_status', array('id' => $status_id), 'name'));
    }
    ...
    public function getAllJobs($client_id, $courier_id = -1, $fulfilled = 0, $store_order = -1, $state = "")
    {
        $db = Database::openConnection();
        $status_id = $this->fulfilled_id;
        $array = array();
